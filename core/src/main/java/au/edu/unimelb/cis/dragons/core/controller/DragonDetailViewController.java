...
public class DragonDetailViewController extends ViewController {
	...
	protected Group createInterface() {
		MigGroup group = new MigGroup(new MigLayout("insets 0, gap 0, fill", "[50%, fill][50%, fill]", "[90%, fill][10%, fill]"));
		...
		MigGroup rightPane = new MigGroup(new MigLayout("insets 5, gap 5, fill", "[][]", "[][][grow]"));
		
		// Add a label that displays the dragon's name
		rightPane.add(new Label("Name:"), "cell 0 0");
		Label dragonNameLabel = new Label();
		...
		rightPane.add(new Label("State:"), "cell 0 1");
		Label dragonStateLabel = new Label();
